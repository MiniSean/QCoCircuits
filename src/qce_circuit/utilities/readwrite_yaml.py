...
def get_yaml_file_path(filename: str) -> Path:
    """Returns yaml file path as used by read/write functions."""
    return Path(os.path.join(YAML_ROOT, filename))


def read_yaml(filename: str) -> Union[dict, FileNotFoundError]:
...
def write_yaml(filename: str, packable: dict, make_file: bool = False, *args, **kwargs) -> bool:
    ...
    with open(file_path, 'w') as f:
        yaml.dump(packable, f, default_flow_style=False, *args, **kwargs)
    return True
